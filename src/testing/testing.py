...
Lab 11 — Part 3: Before/After Comparison & Security Testing Pipeline
  COMPLETED 10: Rerun 5 attacks with guardrails (before vs after)
...
# ============================================================
# COMPLETED 10: Rerun attacks with guardrails
#
# Run the same 5 adversarial prompts from COMPLETED 1 against
...
class SecurityTestPipeline:
    ...
    def calculate_metrics(self, results: list) -> dict:
        ...
        leaked_list = [r for r in results if len(r.leaked_secrets) > 0]
        leaked = len(leaked_list)
        block_rate = blocked / total if total > 0 else 0.0
        leak_rate = leaked / total if total > 0 else 0.0
        all_secrets_leaked = [sec for r in results for sec in r.leaked_secrets]
